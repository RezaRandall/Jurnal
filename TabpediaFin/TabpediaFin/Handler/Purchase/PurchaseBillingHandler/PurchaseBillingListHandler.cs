...
{
    public class PurchaseBillingListHandler : IFetchPagedListHandler<PurchaseBillingListDto>
    {
        private readonly DbManager _dbManager;
        private readonly ICurrentUser _currentUser;

        public PurchaseBillingListHandler(DbManager dbManager, ICurrentUser currentUser)
        {
            _dbManager = dbManager;
            _currentUser = currentUser;
        }

        public async Task<PagedListResponse<PurchaseBillingListDto>> Handle(FetchPagedListRequestDto<PurchaseBillingListDto> request, CancellationToken cancellationToken)
        {
            if (request.PageNum == 0) { request.PageNum = 0; }
            if (request.PageSize == 0) { request.PageSize = 10; }
            var response = new PagedListResponse<PurchaseBillingListDto>();
            try
            {
                using (var cn = _dbManager.CreateConnection())
                {
                    ...
                    var sql = @"SELECT  c.""Name"" as Vendor, 
                                        ...
                                        i.""Memo"",
                                        i.""Notes""  
                                        FROM ""PurchaseBilling"" i LEFT JOIN ""AppUser"" a ON a.""Id"" = i.""StaffId"" LEFT JOIN ""Contact"" c ON c.""Id"" = i.""VendorId""  WHERE i.""TenantId"" = @TenantId " + contactfilter + " " + sqlsearch + " " + sqlsort + " LIMIT @PageSize OFFSET @PageNum";
                    ...
                    List<PurchaseBillingListDto> result;
                    result = (await cn.QueryAsync<PurchaseBillingListDto>(sql, parameters).ConfigureAwait(false)).ToList();
    ...
    [Table("PurchaseBilling")]
    public class PurchaseBillingListDto : BaseDto
    {
        [Searchable]
