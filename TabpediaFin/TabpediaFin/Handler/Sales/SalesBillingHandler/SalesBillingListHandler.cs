...
{
    public class SalesBillingListHandler : IFetchPagedListHandler<SalesBillingListDto>
    {
        private readonly DbManager _dbManager;
        private readonly ICurrentUser _currentUser;

        public SalesBillingListHandler(DbManager dbManager, ICurrentUser currentUser)
        {
            _dbManager = dbManager;
            _currentUser = currentUser;
        }

        public async Task<PagedListResponse<SalesBillingListDto>> Handle(FetchPagedListRequestDto<SalesBillingListDto> request, CancellationToken cancellationToken)
        {
            if (request.PageNum == 0) { request.PageNum = 0; }
            if (request.PageSize == 0) { request.PageSize = 10; }
            var response = new PagedListResponse<SalesBillingListDto>();
            try
            {
                using (var cn = _dbManager.CreateConnection())
                {
                    ...
                    var sql = @"SELECT  c.""Name"" as Vendor, 
                                        ...
                                        i.""Memo"",
                                        i.""Notes""  
                                        FROM ""SalesBilling"" i LEFT JOIN ""AppUser"" a ON a.""Id"" = i.""StaffId"" LEFT JOIN ""Contact"" c ON c.""Id"" = i.""VendorId""  WHERE i.""TenantId"" = @TenantId " + contactfilter + " " + sqlsearch + " " + sqlsort + " LIMIT @PageSize OFFSET @PageNum";
                    ...
                    List<SalesBillingListDto> result;
                    result = (await cn.QueryAsync<SalesBillingListDto>(sql, parameters).ConfigureAwait(false)).ToList();
    ...
    [Table("SalesBilling")]
    public class SalesBillingListDto : BaseDto
    {
        [Searchable]
