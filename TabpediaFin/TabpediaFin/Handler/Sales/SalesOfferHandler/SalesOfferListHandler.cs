...
{
    public class SalesOfferListHandler : IFetchPagedListHandler<SalesOfferListDto>
    {
        private readonly DbManager _dbManager;
        private readonly ICurrentUser _currentUser;

        public SalesOfferListHandler(DbManager dbManager, ICurrentUser currentUser)
        {
            _dbManager = dbManager;
            _currentUser = currentUser;
        }

        public async Task<PagedListResponse<SalesOfferListDto>> Handle(FetchPagedListRequestDto<SalesOfferListDto> request, CancellationToken cancellationToken)
        {
            if (request.PageNum == 0) { request.PageNum = 0; }
            if (request.PageSize == 0) { request.PageSize = 10; }
            var response = new PagedListResponse<SalesOfferListDto>();
            try
            {
                using (var cn = _dbManager.CreateConnection())
                {
                    ...
                    var sql = @"SELECT  c.""Name"" as Vendor, 
                                        ...
                                        i.""Memo"",
                                        i.""Notes""  
                                        FROM ""SalesOffer"" i LEFT JOIN ""AppUser"" a ON a.""Id"" = i.""StaffId"" LEFT JOIN ""Contact"" c ON c.""Id"" = i.""VendorId""  WHERE i.""TenantId"" = @TenantId " + contactfilter + " " + sqlsearch + " " + sqlsort + " LIMIT @PageSize OFFSET @PageNum";
                    ...
                    List<SalesOfferListDto> result;
                    result = (await cn.QueryAsync<SalesOfferListDto>(sql, parameters).ConfigureAwait(false)).ToList();
    ...
    [Table("SalesOffer")]
    public class SalesOfferListDto : BaseDto
    {
        [Searchable]
