...
{
    public class SalesOrderListHandler : IFetchPagedListHandler<SalesOrderListDto>
    {
        private readonly DbManager _dbManager;
        private readonly ICurrentUser _currentUser;

        public SalesOrderListHandler(DbManager dbManager, ICurrentUser currentUser)
        {
            _dbManager = dbManager;
            _currentUser = currentUser;
        }

        public async Task<PagedListResponse<SalesOrderListDto>> Handle(FetchPagedListRequestDto<SalesOrderListDto> request, CancellationToken cancellationToken)
        {
            if (request.PageNum == 0) { request.PageNum = 0; }
            if (request.PageSize == 0) { request.PageSize = 10; }
            var response = new PagedListResponse<SalesOrderListDto>();
            try
            {
                using (var cn = _dbManager.CreateConnection())
                {
                    ...
                    var sql = @"SELECT  c.""Name"" as Vendor, 
                                        ...
                                        i.""Memo"",
                                        i.""Notes""  
                                        FROM ""SalesOrder"" i LEFT JOIN ""AppUser"" a ON a.""Id"" = i.""StaffId"" LEFT JOIN ""Contact"" c ON c.""Id"" = i.""VendorId""  WHERE i.""TenantId"" = @TenantId " + contactfilter + " " + sqlsearch + " " + sqlsort + " LIMIT @PageSize OFFSET @PageNum";
                    ...
                    List<SalesOrderListDto> result;
                    result = (await cn.QueryAsync<SalesOrderListDto>(sql, parameters).ConfigureAwait(false)).ToList();
    ...
    [Table("SalesOrder")]
    public class SalesOrderListDto : BaseDto
    {
        [Searchable]
